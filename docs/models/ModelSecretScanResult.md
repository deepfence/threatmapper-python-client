# threatmapper.model.model_secret_scan_result.ModelSecretScanResult

## Model Type Info
Input Type | Accessed Type | Description | Notes
------------ | ------------- | ------------- | -------------
dict, frozendict.frozendict,  | frozendict.frozendict,  |  | 

### Dictionary Keys
Key | Input Type | Accessed Type | Description | Notes
------------ | ------------- | ------------- | ------------- | -------------
**[severity_counts](#severity_counts)** | dict, frozendict.frozendict, None,  | frozendict.frozendict, NoneClass,  |  | 
**node_type** | str,  | str,  |  | 
**docker_container_name** | str,  | str,  |  | 
**updated_at** | decimal.Decimal, int,  | decimal.Decimal,  |  | value must be a 64 bit integer
**kubernetes_cluster_name** | str,  | str,  |  | 
**node_name** | str,  | str,  |  | 
**created_at** | decimal.Decimal, int,  | decimal.Decimal,  |  | value must be a 64 bit integer
**scan_id** | str,  | str,  |  | 
**[secrets](#secrets)** | list, tuple, None,  | tuple, NoneClass,  |  | 
**docker_image_name** | str,  | str,  |  | 
**host_name** | str,  | str,  |  | 
**node_id** | str,  | str,  |  | 
**any_string_name** | dict, frozendict.frozendict, str, date, datetime, int, float, bool, decimal.Decimal, None, list, tuple, bytes, io.FileIO, io.BufferedReader | frozendict.frozendict, str, BoolClass, decimal.Decimal, NoneClass, tuple, bytes, FileIO | any string name can be used but the value must be the correct type | [optional]

# secrets

## Model Type Info
Input Type | Accessed Type | Description | Notes
------------ | ------------- | ------------- | -------------
list, tuple, None,  | tuple, NoneClass,  |  | 

### Tuple Items
Class Name | Input Type | Accessed Type | Description | Notes
------------- | ------------- | ------------- | ------------- | -------------
[**ModelSecret**](ModelSecret.md) | [**ModelSecret**](ModelSecret.md) | [**ModelSecret**](ModelSecret.md) |  | 

# severity_counts

## Model Type Info
Input Type | Accessed Type | Description | Notes
------------ | ------------- | ------------- | -------------
dict, frozendict.frozendict, None,  | frozendict.frozendict, NoneClass,  |  | 

### Dictionary Keys
Key | Input Type | Accessed Type | Description | Notes
------------ | ------------- | ------------- | ------------- | -------------
**any_string_name** | decimal.Decimal, int,  | decimal.Decimal,  | any string name can be used but the value must be the correct type | [optional] 

[[Back to Model list]](../../README.md#documentation-for-models) [[Back to API list]](../../README.md#documentation-for-api-endpoints) [[Back to README]](../../README.md)

